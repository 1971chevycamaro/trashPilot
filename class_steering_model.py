...
class SteeringWheelModel:
    def __init__(self, angle=0.0, velocity=0.0, torque=0.0, inertia=0.01, sfriction=4, kfriction=2, damping=0.01, centering=0.04, timestep = 0.01):
        """
    ...
    def update(self, dt):
        ...
        for i in range(int(dt//self.timestep)):
            ...
            friction_torque = min(abs(-(self.inertia*self.velocity)/self.timestep), self.kfriction) * -np.sign(self.velocity)
